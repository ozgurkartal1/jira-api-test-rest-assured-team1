	...
	public void theResponseFeaturesOfProjectsShouldMatchWithFeaturesInJsonFile() {
		...
		actualProjects.forEach(project -> {
			softAssertions.assertThat(project.getId())
				.isEqualTo(expectedProjects.get(actualProjects.indexOf(project)).getId());
			softAssertions.assertThat(project.getEntityId())
				.isEqualTo(expectedProjects.get(actualProjects.indexOf(project)).getEntityId());
			softAssertions.assertThat(project.getKey())
				.isEqualTo(expectedProjects.get(actualProjects.indexOf(project)).getKey());
			softAssertions.assertThat(project.getName())
				.isEqualTo(expectedProjects.get(actualProjects.indexOf(project)).getName());
			softAssertions.assertThat(project.getAvatarUrls().getJsonMember16x16())
				.isEqualTo(expectedProjects.get(actualProjects.indexOf(project)).getAvatarUrls().getJsonMember16x16());
		});
